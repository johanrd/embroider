    ...
    project.mergeFiles({
      'ember-cli-build.js': `
        ...
        module.exports = async function (defaults) {
          ...
          // Split 'parent' and 'parent.map' so a route whose name ends in ".map"
          // is dynamically imported in 'vite dev'. That import used to be hijacked
          // by Vite's sourcemap middleware (the id ended in ".map"), so the chunk
          // was served as a sourcemap instead of JS and the route failed to load.
