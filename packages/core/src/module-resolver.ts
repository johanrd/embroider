...
export class Resolver {
  ...
  private handleRouteEntrypoint<R extends ModuleRequest>(request: R): R {
    ...
    // `.map`, which Vite's dev sourcemap middleware would otherwise intercept
    // (a `map` route then 404s in dev). The route rides in the resolver `meta`
    // and nothing decodes this suffix, so the tail is free to change.
